...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for belief in livingGhostPositionDistributions:
            maxposition.append(max(belief,key=lambda x :belief[x]))
        minvalue=float("inf")
        action=None
        for act in legal:
            value=min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act),min_maxposition) for min_maxposition in maxposition)
            if value<=minvalue:
                action=act
                minvalue=value
        return action