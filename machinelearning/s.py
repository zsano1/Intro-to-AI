...
class PerceptronModel(object):
    ...
    def train(self, dataset):
        ...
        batch_size = 1
        while True:
            correct_predice = True
            for x, y in dataset.iterate_once(batch_size):
                if self.get_prediction(x)!=nn.as_scalar(y):
                    correct_predice=False
                    self.w.update(x, nn.as_scalar(y))
            if correct_predice==True:
                break
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        hidden1_o=nn.AddBias(hidden1,self.b1)
        out=nn.Linear(nn.ReLU(hidden1_o),self.w2)
        out_o=nn.AddBias(out,self.b2)
        return out_o
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        # Initialize your model parameters here
        "*** YOUR CODE HERE ***"
        self.w=nn.Parameter(self.num_chars,150)
        self.b=nn.Parameter(1,150)
        self.w_hidden=nn.Parameter(150,150)
        self.b_hidden=nn.Parameter(1,150)
        self.w_last=nn.Parameter(150,5)  #Used at the last layer to match the output
    ...
    def train(self, dataset):
        ...
        for epoch in range(0,90):
            for x,y in dataset.iterate_once(25):
                loss=self.get_loss(x,y)
                g_w,g_w_hidden,g_w_last,g_b,g_b_h,g_b_last= nn.gradients(loss, [self.w,self.w_hidden,self.w_last,self.b,self.b_hidden,self.b_last])
                self.w.update(g_w,-0.002)
                self.w_hidden.update(g_w_hidden,-0.0015)
                self.w_last.update(g_w_last,-0.0015)
                self.b.update(g_b,-0.0015)
                self.b_hidden.update(g_b_h,-0.0015)
                self.b_last.update(g_b_last,-0.015)
